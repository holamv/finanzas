...
export const PredictionsDashboard: React.FC<PredictionsDashboardProps> = ({
  ...
  const mlStats = useMemo(() => {
    ...
    // Filtrar por país. Si es Global, tomamos todos y convertimos.
    const rawCountryData = selectedCountry === 'Global' ? data : data.filter(d => d.country === selectedCountry);
    if (rawCountryData.length === 0) return null;

    // Convertir a USD si es Global
    const countryData = rawCountryData.map(d => ({
      ...d,
      convertedAmount: selectedCountry === 'Global'
        ? convertToUSD(d.amount, (d.moneda && d.moneda !== 'USD' ? d.moneda : d.PAIS) || 'USD')
        : d.amount
    }));
    ...
    const total21Days = last21DaysSales.reduce((sum, d) => sum + d.convertedAmount, 0);
